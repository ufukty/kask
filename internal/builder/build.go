...
func execPage(dst string, tmpl *template.Template, content *TemplateContent) error {
	f, err := os.Create(dst)
	if err != nil {
...
func (b *builder) execDir(d *dir2, dst string, root *Node) error {
	err := os.MkdirAll(filepath.Join(dst, d.SrcPath), 0755)
	if err != nil {
		return fmt.Errorf("creating directory: %w", err)
	}

	for _, page := range d.PagesHtml {
		dst2 := filepath.Join(dst, page)
		...
		if err := execPage(dst2, d.Tmpl, content); err != nil {
			return fmt.Errorf("page %q: %w", filepath.Base(page), err)
		}
	}

	for _, page := range d.PagesMarkdown {
		dst2 := filepath.Join(dst, d.SrcPath)
		if filepath.Base(page) != "README.md" {
			dst2 = filepath.Join(dst, strings.TrimRight(filepath.Base(page), ".md")+".html")
		}
		content := &TemplateContent{
			Stylesheets: d.Stylesheets,
			Node:        nil,
	...
	for _, subdir := range d.Subdirs {
		if err := b.execDir(subdir, dst, root); err != nil {
			return fmt.Errorf("%s: %w", subdir.SrcName, err)
		}
	}
...
func (b *builder) Build(dst, src string) error {
	...
	if err := b.execDir(root2, dst, root3); err != nil {
